 ...
 * matrixcube in R, geared for use with the htmlwidgets and shiny packages.
 */
HTMLWidgets.widget(
{
...
// add the points
  ...
  if(GL)
  {
    if(x.options.renderer=="webgl-buffered")
    {
      ...
      geometry.applyMatrix( new THREE.Matrix4().makeTranslation( 5,5,5)); // SDWF
  ...
  function tick(length, thickness, axis, ticks, ticklabels)
  {
    for(var j=0; j<ticks.length; j++)
    {
      ...
      if(ticklabels)
        ...
        addText(group, ticklabels[j], 0.5, a3, b3, c3, "#555");
...
// Grid
  ...
  {
      var mouse3D = new THREE.Vector3( ( event.clientX / window.innerWidth ) * 2 - 1,   -( event.clientY / window.innerHeight ) * 2 + 1,  0.5 ).unproject(obj.camera);
      var raycaster=new THREE.Raycaster(obj.camera.position, mouse3D.sub(obj.camera.position).normalize());
      raycaster.params.PointCloud.threshold = 0.01
